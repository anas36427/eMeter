...
/**
 * Import the offline sync JSON file generated by the web admin panel.
 * Seeds users, consumers, and settings for Total Offline Mode.
 */
export const importSyncFile = async (fileUri) => {
    try {
        const content = await FileSystem.readAsStringAsync(fileUri, { encoding: FileSystem.EncodingType.UTF8 });
        console.log("FILE IMPORT DEBUG - First 100 chars:", content.substring(0, 100));
        const data = JSON.parse(content);
        
        if (!data.version || !data.consumers) {
            throw new Error("Invalid sync file format.");
        }

        // 1. Save Consumers
        await cacheConsumersToDb(data.consumers);

        // 2. Save Settings
        if (data.settings) {
            await cacheBillingSettings(data.settings);
        }

        console.log(`✅ Offline Sync complete. Loaded ${data.consumers.length} consumers.`);
        return { success: true, consumers: data.consumers.length };
    } catch (err) {
        console.error('Offline sync import failed:', err);
        throw err;
    }
};
